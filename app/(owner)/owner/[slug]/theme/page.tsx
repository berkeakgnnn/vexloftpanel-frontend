...
const COLOR_FIELDS: { key: keyof BusinessTheme; label: string }[] = [
  ...
  { key: "textColor", label: "Yazı Rengi" },
  { key: "mutedTextColor", label: "Soluk Yazı" },
  { key: "borderColor", label: "Kenar Rengi" },
];

const LAYOUT_OPTIONS = [
  { value: "FULLCARD", label: "Full Card", desc: "Büyük fotoğraflı kartlar" },
  { value: "LIST", label: "Liste", desc: "Metin tabanlı fiyat listesi" },
  { value: "GRID", label: "Grid", desc: "2 sütunlu küçük kartlar" },
  { value: "HYBRID", label: "Karma", desc: "İçkilerde liste, yemeklerde grid" },
] as const;
    ...
    <div className="space-y-2">
      ...
      {value && (
        <div className="relative w-full h-28 rounded-lg overflow-hidden border border-border bg-muted">
          <img src={`${API_URL}${value}`} alt={label} className="w-full h-full object-cover" />
          <button
            type="button"
...
const SAMPLE_ITEMS = [
  { name: "Espresso", price: 90, desc: "Yoğun italyan kahvesi" },
  { name: "Cappuccino", price: 120, desc: "Kremalı köpüklü kahve" },
  { name: "Latte", price: 130, desc: "Sütlü yumuşak kahve" },
  { name: "Americano", price: 100, desc: "Klasik filtre tadı" },
];
    ...
    <>
      ...
      <div className="px-4 pt-4">
        ...
        {SAMPLE_ITEMS.map((item, i) => (
          <div key={i} className="flex items-baseline gap-1 mb-3">
            <span
...
function LayoutPreview({ form }: { form: Partial<BusinessTheme> }) {
  ...
  return (
    <div className="w-[280px] h-[520px] rounded-[2rem] border-[6px] border-gray-800 overflow-hidden shadow-2xl">
      <div
...
export default function OwnerThemePage() {
  ...
  const handleSave = () => {
    update.mutate(form, {
      onSuccess: () => toast.success("Tema güncellendi"),
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Tema Editörü</h1>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-[400px] shrink-0 space-y-4">
          ...
          <Button
            ...
            className="w-full h-11 text-base bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-700 hover:to-indigo-600 text-white border-0"
          >
        ...
        <div className="flex-1 min-h-[500px]">
          <Card className="h-full">
            <CardHeader className="pb-2">
              <CardTitle className="text-base font-semibold">Canlı Önizleme</CardTitle>
            </CardHeader>
